...
$checks = @(
    ...
    @{ Label = "collect_pc_inventory.ps1"; Path = (Join-Path $BundleRoot "scripts\collect_pc_inventory.ps1") },
    @{ Label = "install_user_startup_backend.ps1"; Path = (Join-Path $BundleRoot "scripts\install_user_startup_backend.ps1") },
    @{ Label = "plant-pc-deployment.md"; Path = (Join-Path $BundleRoot "docs\handover\plant-pc-deployment.md") },
    @{ Label = "release-bundle.md"; Path = (Join-Path $BundleRoot "docs\handover\release-bundle.md") },
    @{ Label = "pilot-evidence-log.md"; Path = (Join-Path $BundleRoot "docs\handover\pilot-evidence-log.md") }
)
